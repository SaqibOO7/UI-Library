...
import { serverUrl } from '../../../../../AI Interviewer App/IntView/Frontend/src/App'
...
function Pricing() {
  ...
  const handlePayment = async (plan) => {
    try {
      ...
      const result = await axios.post(serverUrl + "/api/v1/payment/create",
        {
          amount,
      ...
      const options = {
        ...
        handler: async function (response) {
          const verify = await axios.post(serverUrl + "api/v1/payment/verify", response, { withCredentials: true })
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden flex flex-col"
      ...
      <div className="absolute top-[-8%] left-[10%] w-80 h-80 rounded-full pointer-events-none opacity-20"
        style={{
          background: "radial-gradient(circle, #6366f1 0%, transparent 70%)", filter: "blur(70px)"
        }}
      />

      <div className="absolute bottom-[-6%] right-[5%] w-72 h-72 rounded-full pointer-events-none opacity-15"
        style={{
          background: "radial-gradient(circle, #06b6d4 0%, transparent 70%)", filter: "blur(60px)"
        }}
      />
      ...
      <div className='relative z-10 max-w-3xl mx-auto px-4 py-14 w-full'>
        <motion.button
          ...
          className='flex items-center gap-2 text-sm text-white/40 hover:text-white/70 transition-all mb-10
              ...
              <ul className="space-y-2.5 mb-8 flex-1">
                {plan.features.map((f) => (
                  <li key={f} className='flex items-center gap-2.5 text-sm text-white/60'>
                    <FiCheck size={13} style={{
                      color: plan.
                        highlight ? "#818cf8" : "rgba(255,255,255,0.3)"
                    }} />
                    {f}
                  </li>
                ))}
