                ...
                {profileOpen && (
                  ...
                  >
                    ...
                    <div className='py-1.5'>
                      ...
                      <button
                        onClick={() => { navigate('/submit-for-approval'); setProfileOpen(false) }}
                        className='w-full flex items-center gap-3 px-4 py-2.5 text-sm text-white/60 hover:text-white
                      hover:bg-white/[0.04] transition-colors cursor-pointer bg-transparent border-none text-left'>

                        <TbArrowRight size={16} className='text-[#3be8ff]/70' /> Submit for Approval
                      </button>

                      <button
                        onClick={() => { navigate('/my-submissions'); setProfileOpen(false) }}
                        className='w-full flex items-center gap-3 px-4 py-2.5 text-sm text-white/60 hover:text-white
                      hover:bg-white/[0.04] transition-colors cursor-pointer bg-transparent border-none text-left'>

                        <TbCheck size={16} className='text-[#3be8ff]/70' /> My Submissions
                      </button>

                      {userData?.role === 'admin' && (
                        <button
                          onClick={() => { navigate('/admin-review'); setProfileOpen(false) }}
                          className='w-full flex items-center gap-3 px-4 py-2.5 text-sm text-amber-400/80 hover:text-amber-400
                        hover:bg-amber-500/[0.06] transition-colors cursor-pointer bg-transparent border-none text-left'>

                          <TbAdjustments size={16} /> Review Submissions
                        </button>
                      )}
                    </div>
