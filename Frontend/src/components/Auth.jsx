    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className='flex flex-col sm:flex-row w-full max-w-[880px] max-h-[90vh] overflow-y-auto rounded-2xl border
        border-[#3be8ff]/10 shadow-[0_40px_80px_rgba(0,0,0,0.8)] relative'
        >

          {/* X button */}
          <button
            onClick={onClose}
            className='absolute top-3 right-3 z-20 w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10
          border border-white/10 flex items-center justify-center text-white/50 hover:text-white transition-all'>
            <TbX size={15} />
          </button>
            ...
            <div className='flex sm:flex-col gap-2 sm:gap-1 overflow-x-auto sm:overflow-x-visible pb-2 sm:pb-0 -mx-1 px-1'>
              {
                steps.map((item, i) => (
                  <motion.div
                    key={i}
                    className={`flex-shrink-0 sm:flex-shrink flex items-start gap-3 px-3 py-2.5 rounded-xl border transition-all
                      duration-300 min-w-[200px] sm:min-w-0 
                      ${active === i ? "bg-[#3be8ff]/[0.07] border-[#3be8ff]" : "bg-transparent border-transparent"}`}
                  >
                    <div className={`min-w-[28px] h-7 rounded-lg flex items-center justify-center border transition-all
                      duration-300 ${active === i ? "bg-gradient-to-br from-[#3be8ff] to-[#0ab8d6] border-transparent"
                    ...
                    <div className="">
                      <p className={`text-[12.5px] font-semibold transition-colors duration-300 whitespace-nowrap sm:whitespace-normal
                      ...
                      <div className={`overflow-hidden transition-all duration-500 
                        ...
                        <p className="text-[11px] text-[#3be8ff]/40 mt-0.5 leading-relaxed">
                          {item.desc}
                        </p>
                      </div>
                    </div>
            ...
            <div className='relative z-10 w-full max-w-[280px] sm:max-w-[260px] text-center mx-auto'>
              ...
              <p className='text-[11px] text-[#64919b]/45 mt-4 sm:mt-5'>
                ...
                <span onClick={onClose}
                  ...
                  hover:text-[#3be8ff]/80 transition-colors'>
                  View docs ➔
                </span>
              </p>
            </div>
